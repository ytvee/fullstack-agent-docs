# About the shadow database (/docs/v6/orm/prisma-migrate/understanding-prisma-migrate/shadow-database)
...
6. Drops the shadow database (shadow databases configured via [`shadowDatabaseUrl`](/v6/orm/reference/prisma-schema-reference#datasource) are not dropped, but are reset at the start of the `migrate dev` command)
...
<CodeBlockTabs defaultValue="Prisma 7 (prisma.config.ts)">
  <CodeBlockTabsList>
    <CodeBlockTabsTrigger value="Prisma 7 (prisma.config.ts)">
      Prisma 7 (prisma.config.ts)
    </CodeBlockTabsTrigger>

    <CodeBlockTabsTrigger value="Prisma 6 and below (schema.prisma)">
      Prisma 6 and below (schema.prisma)
    </CodeBlockTabsTrigger>
  </CodeBlockTabsList>

  <CodeBlockTab value="Prisma 7 (prisma.config.ts)">
    ```ts
  ...
  <CodeBlockTab value="Prisma 6 and below (schema.prisma)">
    ```prisma
    datasource db {
      provider          = "postgresql"
      shadowDatabaseUrl = env("SHADOW_DATABASE_URL") // [!code highlight]
