# Database drivers (/docs/v6/orm/overview/databases/database-drivers)
...
In Prisma ORM 7, the database connection URL is configured in [`prisma.config.ts`](/v6/orm/reference/prisma-config-reference). However, when using a driver adapter, the connection string needs to be provided in your *application code* when the driver adapter is set up initially.
