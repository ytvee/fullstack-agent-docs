# Connection pooling (/docs/v6/postgres/database/connection-pooling)
...
Connection pooling in Prisma Postgres [#connection-pooling-in-prisma-postgres]
...
Configuring the connection pool size [#configuring-the-connection-pool-size]
...
Configuring query limits [#configuring-query-limits]
...
Query timeout limit [#query-timeout-limit]
...
Interactive transactions query timeout limit [#interactive-transactions-query-timeout-limit]
...
Autoscaling (Accelerate + Your own database only) [#autoscaling-accelerate--your-own-database-only]
