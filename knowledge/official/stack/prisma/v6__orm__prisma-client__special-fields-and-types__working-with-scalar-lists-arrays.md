# Working with scalar lists (/docs/v6/orm/prisma-client/special-fields-and-types/working-with-scalar-lists-arrays)
...
Setting the value of a scalar list [#setting-the-value-of-a-scalar-list]
...
Unsetting the value of a scalar list [#unsetting-the-value-of-a-scalar-list]
...
Adding items to a scalar list [#adding-items-to-a-scalar-list]
...
Filtering scalar lists [#filtering-scalar-lists]
...
Use [scalar list filters](/v6/orm/reference/prisma-client-reference#scalar-list-filters) to filter for records with scalar lists that match a specific condition. The following example returns all posts where the tags list includes `databases` *and* `typescript`:
