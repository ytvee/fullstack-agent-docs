# `query`: Create custom Prisma Client queries (/docs/v6/orm/prisma-client/client-extensions/query)
...
You can use the `query` [Prisma Client extensions](/v6/orm/prisma-client/client-extensions) component type to hook into the query life-cycle and modify an incoming query or its result.

You can use Prisma Client extensions `query` component to create independent clients with customized behavior. You can bind one client to a specific filter or user, and another client to another filter or user. For example, you might do this to get [user isolation](/v6/orm/prisma-client/client-extensions#extended-clients) in a row-level security (RLS) extension. The `query` extension component provides end-to-end type safety for all your custom queries.

Extend Prisma Client query operations [#extend-prisma-client-query-operations]

Use the `$extends` [client-level method](/v6/orm/reference/prisma-client-reference#client-methods) to create an [extended client](/v6/orm/prisma-client/client-extensions#about-prisma-client-extensions). An extended client is a variant of the standard Prisma Client that is wrapped by one or more extensions.
...
The `query` object can contain functions that map to the names of the [Prisma Client operations](/v6/orm/reference/prisma-client-reference#model-queries), such as `findUnique()`, `findFirst`, `findMany`, `count`, and `create`. The following example modifies `user.findMany` to a use a customized query that finds only users who are older than 18 years:
...
In the event a [raw query](/v6/orm/prisma-client/using-raw-sql/raw-queries) is invoked, the `model` argument passed to the callback will be `undefined`.
...
<CalloutContainer type="info">
  <CalloutDescription>
    We include the above example to show that this is possible. However, for performance reasons we recommend that you use the [`result` component type](/v6/orm/prisma-client/client-extensions/result) to override existing fields. The `result` component type usually gives better performance in this situation because it computes only on access. The `query` component type computes after query execution.
  </CalloutDescription>
</CalloutContainer>

Wrap a query into a batch transaction [#wrap-a-query-into-a-batch-transaction]

You can wrap your extended queries into a [batch transaction](/v6/orm/prisma-client/queries/transactions). For example, you can use this to enact row-level security (RLS).
