# SQLite (/docs/v6/orm/overview/databases/sqlite)
...
By default, the SQLite connector contains a database driver responsible for connecting to your database. You can use a [driver adapter](/v6/orm/overview/databases/database-drivers#driver-adapters) (Preview) to connect to your database using a JavaScript database driver from Prisma Client.

Example [#example]

To connect to a SQLite database file, you need to configure a [`datasource`](/v6/orm/prisma-schema/overview/data-sources) block in your [Prisma schema](/v6/orm/prisma-schema/overview):
...
Using the better-sqlite3 driver [#using-the-better-sqlite3-driver]
...
3. Configure timestamp format for backward compatibility [#3-configure-timestamp-format-for-backward-compatibility]
...
Type mapping between SQLite to Prisma schema [#type-mapping-between-sqlite-to-prisma-schema]

The SQLite connector maps the [scalar types](/v6/orm/prisma-schema/data-model/models#scalar-fields) from the [data model](/v6/orm/prisma-schema/data-model/models) to native column types as follows:
...
Native type mapping from Prisma ORM to SQLite [#native-type-mapping-from-prisma-orm-to-sqlite]
...
Rounding errors on big numbers [#rounding-errors-on-big-numbers]
