# `result`: Add custom fields and methods to query results (/docs/v6/orm/prisma-client/client-extensions/result)
...
You can use the `result` [Prisma Client extensions](/v6/orm/prisma-client/client-extensions) component type to add custom fields and methods to query results.

Use the `$extends` [client-level method](/v6/orm/reference/prisma-client-reference#client-methods) to create an *extended client*. An extended client is a variant of the standard Prisma Client that is wrapped by one or more extensions.
...
Add a custom field to query results [#add-a-custom-field-to-query-results]
...
Re-use a computed field in another computed field [#re-use-a-computed-field-in-another-computed-field]
...
Add a custom method to the result object [#add-a-custom-method-to-the-result-object]
...
Using omit query option with result extension component [#using-omit-query-option-with-result-extension-component]

You can use the [`omit` (Preview) option](/v6/orm/reference/prisma-client-reference#omit) with [custom fields](#add-a-custom-field-to-query-results) and fields needed by custom fields.
