# Better Auth (with Next.js) (/docs/v6/guides/betterauth-nextjs)
...
<CodeBlockTabs defaultValue="npm" groupId="package-manager" persist>
  ...
  <CodeBlockTab value="bun">
    ```bash
    bunx --bun create-next-app@latest betterauth-nextjs-prisma
    ```
  </CodeBlockTab>
</CodeBlockTabs>
...
export default function DashboardPage() {
  ...
  if (isPending) return <p className="text-center mt-8 text-white">Loading...</p>; // [!code ++]
  if (!session?.user) return <p className="text-center mt-8 text-white">Redirecting...</p>; // [!code ++]
