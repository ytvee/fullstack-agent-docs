# Prototyping your schema (/docs/v6/orm/prisma-migrate/workflows/prototyping-your-schema)
...
Choosing db push or Prisma Migrate [#choosing-db-push-or-prisma-migrate]
...
Can I use Prisma Migrate and db push together? [#can-i-use-prisma-migrate-and-db-push-together]
...
Prototyping a new schema [#prototyping-a-new-schema]
...
8. Initialize a migration history:
   ...
   <CodeBlockTabs defaultValue="npm" groupId="package-manager" persist>
     ...
     <CodeBlockTab value="pnpm">
       ```bash
       pnpm dlx prisma migrate dev --name initial-state
       ```
     </CodeBlockTab>

     <CodeBlockTab value="yarn">
       ```bash
       yarn dlx prisma migrate dev --name initial-state
       ```
     </CodeBlockTab>

     <CodeBlockTab value="bun">
       ```bash
       bunx --bun prisma migrate dev --name initial-state
       ```
     </CodeBlockTab>
   </CodeBlockTabs>
...
3. To create a migration that adds the new `tags` field, run the `migrate dev` command:
   ...
   <CodeBlockTabs defaultValue="npm" groupId="package-manager" persist>
     ...
     <CodeBlockTab value="pnpm">
       ```bash
       pnpm dlx prisma migrate dev --name added-tags
       ```
     </CodeBlockTab>

     <CodeBlockTab value="yarn">
       ```bash
       yarn dlx prisma migrate dev --name added-tags
       ```
     </CodeBlockTab>

     <CodeBlockTab value="bun">
       ```bash
       bunx --bun prisma migrate dev --name added-tags
       ```
     </CodeBlockTab>
   </CodeBlockTabs>
