# Prisma ORM MongoDB database connector (/docs/v6/orm/overview/databases/mongodb)
...
<CalloutContainer type="warning">
  ...
  <CalloutDescription>
    ...
    * [Quickstart with MongoDB](/v6/prisma-orm/quickstart/mongodb)
    * [Add to existing MongoDB project](/v6/prisma-orm/add-to-existing-project/mongodb)
  </CalloutDescription>
</CalloutContainer>
...
* connect to your database, using the [`mongodb` database connector](/v6/orm/overview/databases/database-drivers)
* use [Introspection](/v6/orm/prisma-schema/introspection) for existing projects if you already have a MongoDB database
* use [`db push`](/v6/orm/prisma-migrate/workflows/prototyping-your-schema) to push changes in your schema to the database
* use [Prisma Client](/v6/orm/prisma-client/setup-and-configuration/introduction) in your application to query your database in a type safe way based on your Prisma Schema
...
How to use Prisma ORM with MongoDB [#how-to-use-prisma-orm-with-mongodb]
...
For more information on how to use relations in Prisma ORM, see [our documentation](/v6/orm/prisma-schema/data-model/relations).
...
To include missing fields as well, use the [`isSet` filter](/v6/orm/reference/prisma-client-reference#isset) to explicitly search for fields which are either `null` or not set. This will return both records:
...
More on using MongoDB with Prisma ORM [#more-on-using-mongodb-with-prisma-orm]

The fastest way to start using MongoDB with Prisma ORM is to refer to our Getting Started documentation:

* [Start from scratch](/v6/prisma-orm/quickstart/mongodb)
* [Add to existing project](/v6/prisma-orm/add-to-existing-project/mongodb)
...
Further reference information is available in the [MongoDB connector documentation](/v6/orm/overview/databases/mongodb).
...
To connect to a MongoDB server, configure the [`datasource`](/v6/orm/prisma-schema/overview/data-sources) block in your [Prisma Schema](/v6/orm/prisma-schema/overview):
...
* `url`: Specifies the [connection URL](#connection-url) for the MongoDB server. In this case, an [environment variable is used](/v6/orm/more/dev-environment/environment-variables) to provide the connection URL.
...
<img alt="Structure of the MongoDB connection URL" src="/img/v6/orm/overview/databases/mongodb.png" width="1446" height="256" />
...
See also: [Defining ID fields in MongoDB](/v6/orm/prisma-schema/data-model/models#defining-ids-in-mongodb)
...
Differences to connectors for relational databases [#differences-to-connectors-for-relational-databases]
...
Currently, there are no plans to add support for [Prisma Migrate](/v6/orm/prisma-migrate/getting-started) as MongoDB projects do not rely on internal schemas where changes need to be managed with an extra tool. Management of `@unique` indexes is realized through `db push`.

No support for @@id and autoincrement() [#no-support-for-id-and-autoincrement]

The [`@@id`](/v6/orm/reference/prisma-schema-reference#id-1) attribute (an ID for multiple fields) is not supported because primary keys in MongoDB are always on the `_id` field of a model.

The [`autoincrement()`](/v6/orm/reference/prisma-schema-reference#generate-autoincrementing-integers-as-ids-relational-databases-only) function (which creates incrementing `@id` values) is not supported because `autoincrement()` does not work with the `ObjectID` type that the `_id` field has in MongoDB.

Cyclic references and referential actions [#cyclic-references-and-referential-actions]

If you have cyclic references in your models, either from self-relations or a cycle of relations between models, and you use [referential actions](/v6/orm/prisma-schema/data-model/relations/referential-actions), you must set a referential action of `NoAction` to prevent an infinite loop of actions.

See [Special rules for referential actions](/v6/orm/prisma-schema/data-model/relations/referential-actions/special-rules-for-referential-actions) for more details.
...
Type mapping between MongoDB and the Prisma schema [#type-mapping-between-mongodb-and-the-prisma-schema]

The MongoDB connector maps the [scalar types](/v6/orm/prisma-schema/data-model/models#scalar-fields) from the Prisma ORM [data model](/v6/orm/prisma-schema/data-model/models) to MongoDB's native column types as follows:
...
When introspecting a MongoDB database, Prisma ORM uses the relevant [scalar types](/v6/orm/prisma-schema/data-model/models#scalar-fields). Some special types also get additional native type annotations:
