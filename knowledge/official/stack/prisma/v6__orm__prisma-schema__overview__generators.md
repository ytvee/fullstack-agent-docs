# Generators (/docs/v6/orm/prisma-schema/overview/generators)
...
3. Exclude the generated directory from version control [#3-exclude-the-generated-directory-from-version-control]

The new generator includes both the TypeScript client code *and* the [query engine](/v6/orm/more/internals/engines#the-query-engine-file). Including the query engine in version control can cause compatibility issues on different machines. To avoid this, add the generated directory to `.gitignore`:
...
4. Use Prisma Client in your application [#4-use-prisma-client-in-your-application]
...
The native binary target [#the-native-binary-target]
