# Caching queries in Prisma Postgres (/docs/v6/postgres/database/caching)
...
Setting up caching in Prisma Postgres [#setting-up-caching-in-prisma-postgres]
...
1. Enable caching in the Platform Console [#1-enable-caching-in-the-platform-console]

1. Go to the [Platform Console](https://console.prisma.io/?utm_source=docs-v6\&utm_medium=content\&utm_content=postgres) and navigate to your project dashboard
...
2. Install the Accelerate extension [#2-install-the-accelerate-extension]
...
3. Configure Prisma Client with caching [#3-configure-prisma-client-with-caching]

Update your Prisma Client setup to use the Accelerate extension:

<CodeBlockTabs defaultValue="standard">
  <CodeBlockTabsList>
    <CodeBlockTabsTrigger value="standard">
      standard
    </CodeBlockTabsTrigger>

    <CodeBlockTabsTrigger value="edge">
      edge
    </CodeBlockTabsTrigger>
  </CodeBlockTabsList>

  <CodeBlockTab value="standard">
    ```ts
...
4. Start caching your queries [#4-start-caching-your-queries]
...
<img alt="TTL" src="/img/accelerate/ttl.png" width="1427" height="1007" />

TTL is useful for reducing database load and latency for data that does not require frequent updates.

Invalidate the TTL and keep your cached query results up-to-date [#invalidate-the-ttl-and-keep-your-cached-query-results-up-to-date]
...
<img alt="SWR" src="/img/accelerate/swr.png" width="1739" height="1288" />

Invalidate the SWR and keep your cached query results up-to-date [#invalidate-the-swr-and-keep-your-cached-query-results-up-to-date]
...
Selecting a cache strategy [#selecting-a-cache-strategy]
...
Cache strategy using TTL [#cache-strategy-using-ttl]

Use TTL to reduce database load when stale cached data is acceptable.

Use case: Product catalog in e-commerce applications [#use-case-product-catalog-in-e-commerce-applications]
...
Cache strategy using SWR [#cache-strategy-using-swr]
...
Use case: User profile in social media platforms [#use-case-user-profile-in-social-media-platforms]
...
Cache strategy using TTL + SWR [#cache-strategy-using-ttl--swr]
...
<img alt="Diagram showing cache behavior with TTL and SWR: fresh data for 30 seconds, then stale data served while background revalidation occurs." src="/img/accelerate/ttl_and_swr.png" width="2128" height="1288" />

Use case: News articles [#use-case-news-articles]
...
On-demand cache invalidation [#on-demand-cache-invalidation]
...
Default cache strategy [#default-cache-strategy]
