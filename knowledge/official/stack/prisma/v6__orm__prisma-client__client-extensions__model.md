# `model`: Add custom methods to your models (/docs/v6/orm/prisma-client/client-extensions/model)
...
You can use the `model` [Prisma Client extensions](/v6/orm/prisma-client/client-extensions) component type to add custom methods to your models.
...
Use the `$extends` [client-level method](/v6/orm/reference/prisma-client-reference#client-methods) to create an *extended client*. An extended client is a variant of the standard Prisma Client that is wrapped by one or more extensions. Use the `model` extension component to add methods to models in your schema.
...
Call a custom method from another custom method [#call-a-custom-method-from-another-custom-method]
...
Get the current model name at runtime [#get-the-current-model-name-at-runtime]
...
You can improve the type-safety of `model` components in your shared extensions with [type utilities](/v6/orm/prisma-client/client-extensions/type-utilities).
