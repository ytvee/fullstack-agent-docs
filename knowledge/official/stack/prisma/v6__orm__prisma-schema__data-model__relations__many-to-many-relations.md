# Many-to-many relations (/docs/v6/orm/prisma-schema/data-model/relations/many-to-many-relations)
...
* One model that represents the [relation table](#relation-tables), such as `CategoriesOnPosts` (also sometimes called *JOIN*, *link* or *pivot* table) in the underlying database. The fields of a relation table model are both annotated relation fields (`post` and `category`) with a corresponding relation scalar field (`postId` and `categoryId`).
...
Querying an explicit many-to-many [#querying-an-explicit-many-to-many]
...
Querying an implicit many-to-many [#querying-an-implicit-many-to-many]
...
Rules for defining an implicit m-n relation [#rules-for-defining-an-implicit-m-n-relation]
...
Conventions for relation tables in implicit m-n relations [#conventions-for-relation-tables-in-implicit-m-n-relations]
...
Configuring the name of the relation table in implicit many-to-many relations [#configuring-the-name-of-the-relation-table-in-implicit-many-to-many-relations]
...
Querying MongoDB many-to-many relations [#querying-mongodb-many-to-many-relations]
