# Prisma Studio (/docs/v6/orm/tools/prisma-studio)
...
Option 1: Use with any database (no Prisma project required) [#option-1-use-with-any-database-no-prisma-project-required]
...
Option 2: Use with a Prisma ORM project [#option-2-use-with-a-prisma-orm-project]
...
Standalone usage (without Prisma ORM) [#standalone-usage-without-prisma-orm]
...
With a Prisma ORM project [#with-a-prisma-orm-project]
...
Browse your data [#browse-your-data]
...
View and edit records [#view-and-edit-records]
...
Databases supported by Prisma Studio [#databases-supported-by-prisma-studio]

Prisma Studio currently supports the following databases: PostgreSQL, MySQL, and SQLite.

SQLite requirements for Prisma Studio [#sqlite-requirements-for-prisma-studio]
...
<CalloutContainer type="info">
  ...
  <CalloutDescription>
    ...
    <CodeBlockTabs defaultValue="npm" groupId="package-manager" persist>
      ...
      <CodeBlockTab value="pnpm">
        ```bash
        pnpm dlx -p better-sqlite3 -p prisma prisma studio --url file:./my_file.db
        ```
      </CodeBlockTab>

      <CodeBlockTab value="yarn">
        ```bash
        yarn dlx -p better-sqlite3 -p prisma prisma studio --url file:./my_file.db
        ```
      </CodeBlockTab>

      <CodeBlockTab value="bun">
        ```bash
        bunx --bun -p better-sqlite3 -p prisma prisma studio --url file:./my_file.db
        ```
      </CodeBlockTab>
    </CodeBlockTabs>
...
Databases not yet supported [#databases-not-yet-supported]
...
PostgreSQL: "unrecognized configuration parameter 'schema'" error [#postgresql-unrecognized-configuration-parameter-schema-error]
...
Terminal: Failed to run script / Error in Prisma Client request [#terminal-failed-to-run-script--error-in-prisma-client-request]
