# Engines (/docs/v6/orm/more/internals/engines)



<CalloutContainer type="warning">
  <CalloutTitle>
    Prisma ORM v7 is Rust binary-free by default
  </CalloutTitle>
  ...
  <CalloutDescription>
    ...
    * [Driver adapters](/v6/orm/overview/databases/database-drivers#driver-adapters) are required for database connectivity
    * The generated code is smaller and more portable across different JavaScript environments

    This page documents the legacy Rust-based engine architecture. For the modern Rust-free approach (default in v7), see [Prisma ORM without Rust engines](/v6/orm/prisma-client/setup-and-configuration/no-rust-engine).
  </CalloutDescription>
</CalloutContainer>

<CalloutContainer type="info">
  <CalloutTitle>
    Default generator in Prisma ORM v7
  </CalloutTitle>
  ...
  <CalloutDescription>
    ...
    Learn more about the [`prisma-client` generator](/v6/orm/prisma-schema/overview/generators#prisma-client).
  </CalloutDescription>
</CalloutContainer>
...
<img alt="Prisma engine" src="/img/v6/orm/more/internals/typical-flow-query-engine-at-runtime.png" width="1600" height="814" />

Using custom engine libraries or binaries [#using-custom-engine-libraries-or-binaries]
...
When using Rust-based engines, all engine files are automatically downloaded into the `node_modules/@prisma/engines` folder when you install or update `prisma`, the Prisma CLI package. The [query engine](/v6/orm/more/internals/engines) is also copied to the generated Prisma Client when you call `prisma generate`.
...
* [`PRISMA_QUERY_ENGINE_LIBRARY`](/v6/orm/reference/environment-variables-reference#prisma_query_engine_library) (Query engine, library) — **Not supported in Prisma ORM v7**
* [`PRISMA_QUERY_ENGINE_BINARY`](/v6/orm/reference/environment-variables-reference#prisma_query_engine_binary) (Query engine, binary) — **Not supported in Prisma ORM v7**
* [`PRISMA_SCHEMA_ENGINE_BINARY`](/v6/orm/reference/environment-variables-reference#prisma_schema_engine_binary) (Schema engine)
* [`PRISMA_MIGRATION_ENGINE_BINARY`](/v6/orm/reference/environment-variables-reference#prisma_migration_engine_binary) (Migration engine)
* [`PRISMA_INTROSPECTION_ENGINE_BINARY`](/v6/orm/reference/environment-variables-reference#prisma_introspection_engine_binary) (Introspection engine) — **Not supported in Prisma ORM v7**
...
a) The .env file [#a-the-env-file]

Add the environment variable to the [`.env` file](/v6/orm/more/dev-environment/environment-variables).

<CodeBlockTabs defaultValue="Linux, Unix, macOS">
  <CodeBlockTabsList>
    <CodeBlockTabsTrigger value="Linux, Unix, macOS">
      Linux, Unix, macOS
  ...
  <CodeBlockTab value="Linux, Unix, macOS">
    ```text
...
> **Note**: It is possible to [use an `.env` file in a location outside the `prisma` folder](/v6/orm/more/dev-environment/environment-variables).

b) Global environment variable [#b-global-environment-variable]
...
Test your environment variable [#test-your-environment-variable]
...
Hosting engines [#hosting-engines]
...
The [`PRISMA_ENGINES_MIRROR`](/v6/orm/reference/environment-variables-reference#prisma_engines_mirror) environment variable allows you to host engine files via a private server, AWS bucket or other cloud storage.
...
The query engine file [#the-query-engine-file]
...
The query engine at runtime [#the-query-engine-at-runtime]
...
<img alt="Diagram showing the query engine and Node.js at runtime" src="/img/v6/orm/more/internals/query-engine-node-js-at-runtime.png" width="1600" height="298" />
...
The query engine is stopped and the database connections are closed when [`$disconnect()`](/v6/orm/prisma-client/setup-and-configuration/databases-connections/connection-management) is invoked.
...
<img alt="Typical flow of the query engine at run time" src="/img/v6/orm/more/internals/typical-flow-query-engine-at-runtime.png" width="1600" height="814" />

Responsibilities of the query engine [#responsibilities-of-the-query-engine]
...
Debugging the query engine [#debugging-the-query-engine]
...
You can access the logs of the Rust-based query engine by setting the [`DEBUG`](/v6/orm/prisma-client/debugging-and-troubleshooting/debugging) environment variable to `engine`:

```bash
export DEBUG="engine"
```

You can also get more visibility into the SQL queries that are generated by the query engine by setting the [`query` log level](/v6/orm/reference/prisma-client-reference#log-levels) in Prisma Client:
...
Learn more about [Debugging](/v6/orm/prisma-client/debugging-and-troubleshooting/debugging) and [Logging](/v6/orm/prisma-client/observability-and-logging/logging).

Configuring the query engine [#configuring-the-query-engine]
...
Defining the query engine type for Prisma Client [#defining-the-query-engine-type-for-prisma-client]
...
<CalloutContainer type="warning">
  <CalloutDescription>
    The environment variable `PRISMA_CLIENT_ENGINE_TYPE` is not supported in Prisma ORM v7 because Prisma Client now defaults to the TypeScript-based query compiler without Rust engine binaries. See [Prisma ORM without Rust engines](/v6/orm/prisma-client/setup-and-configuration/no-rust-engine) for more information.
  </CalloutDescription>
</CalloutContainer>
...
<CalloutContainer type="info">
  ...
  <CalloutDescription>
    ...
    * From versions [2.20.0](https://github.com/prisma/prisma/releases/2.20.0) to 3.x the `library` engine type was available and used by default by [activating the preview feature flag](/v6/orm/reference/preview-features/client-preview-features#enabling-a-prisma-client-preview-feature) "`nApi`" or using the `PRISMA_FORCE_NAPI=true` environment variable.
...
Defining the query engine type for Prisma CLI [#defining-the-query-engine-type-for-prisma-cli]

When using Rust-based engines, Prisma CLI also uses its own query engine. You can configure it to use the binary version of the query engine by defining the environment variable [`PRISMA_CLI_QUERY_ENGINE_TYPE=binary`](/v6/orm/reference/environment-variables-reference#prisma_cli_query_engine_type).

<CalloutContainer type="warning">
  <CalloutDescription>
    The environment variable `PRISMA_CLI_QUERY_ENGINE_TYPE` is not supported in Prisma ORM v7 because Prisma CLI now defaults to the TypeScript-based query compiler without Rust engine binaries. See [Prisma ORM without Rust engines](/v6/orm/prisma-client/setup-and-configuration/no-rust-engine) for more information.
  </CalloutDescription>
</CalloutContainer>
