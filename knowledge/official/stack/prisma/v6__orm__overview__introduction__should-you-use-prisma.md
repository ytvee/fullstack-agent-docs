# Should you use Prisma ORM? (/docs/v6/orm/overview/introduction/should-you-use-prisma)
...
Prisma ORM likely is a good fit for you if ... [#prisma-orm-likely-is-a-good-fit-for-you-if-]

... you are building a server-side application that talks to a database [#-you-are-building-a-server-side-application-that-talks-to-a-database]
...
Refer to the full list of databases (relational, NoSQL, and NewSQL) that Prisma ORM [supports](/v6/orm/reference/supported-databases).

... you care about productivity and developer experience [#-you-care-about-productivity-and-developer-experience]
...
... you are working in a team [#-you-are-working-in-a-team]

Prisma ORM shines especially when used in collaborative environments.

The declarative [Prisma schema](/v6/orm/prisma-schema/overview) provides an overview of the current state of the database that's easy to understand for everyone. This is a major improvement to traditional workflows where developers have to dig through migration files to understand the current table structure.

[Prisma Client](/v6/orm/prisma-client/setup-and-configuration/introduction)'s minimal API surface enables developers to pick it up quickly without much learning overhead, so onboarding new developers to a team becomes a lot smoother.

The [Prisma Migrate](/v6/orm/prisma-migrate/getting-started) workflows are designed in a way to cover database schema changes in collaborative environments. From the initial schema creation up to the point of deploying schema changes to production and resolving conflicts that were introduced by parallel modifications, Prisma Migrate has you covered.

... you want a tool that holistically covers your database workflows [#-you-want-a-tool-that-holistically-covers-your-database-workflows]
...
* querying (with [Prisma Client](/v6/orm/prisma-client/setup-and-configuration/introduction))
* data modeling (in the [Prisma schema](/v6/orm/prisma-schema/overview))
* migrations (with [Prisma Migrate](/v6/orm/prisma-migrate/getting-started))
* prototyping (via [`prisma db push`](/v6/orm/reference/prisma-cli-reference#db-push))
* seeding (via [`prisma db seed`](/v6/orm/reference/prisma-cli-reference#db-seed))
* visual viewing and editing (with [Prisma Studio](https://www.prisma.io/studio))

... you value type-safety [#-you-value-type-safety]

Prisma ORM is the only *fully* type-safe ORM in the TypeScript ecosystem. The generated Prisma Client ensures typed query results even for partial queries and relations. You can learn more about this in the [type-safety comparison with TypeORM](/v6/orm/more/comparisons/prisma-and-typeorm#type-safety).

... you want to write raw, type-safe SQL [#-you-want-to-write-raw-type-safe-sql]
...
... you want an ORM with a transparent development process, proper maintenance & support [#-you-want-an-orm-with-a-transparent-development-process-proper-maintenance--support]
...
... you want to be part of an awesome community [#-you-want-to-be-part-of-an-awesome-community]
...
Prisma ORM likely is not a good fit for you if ... [#prisma-orm-likely-is-not-a-good-fit-for-you-if-]

... you need full control over all database queries [#-you-need-full-control-over-all-database-queries]

Prisma ORM is an abstraction. As such, an inherent tradeoff of Prisma ORM is a reduced amount of control in exchange for higher productivity. This means, the [Prisma Client API](/v6/orm/prisma-client/setup-and-configuration/introduction) might have less capabilities in some scenarios than you get with plain SQL.
...
*Alternatives*: SQL drivers (e.g. [`node-postgres`](https://node-postgres.com/), [`mysql`](https://github.com/mysqljs/mysql), [`sqlite3`](https://github.com/TryGhost/node-sqlite3), ...)

... you do not want to write any code for your backend [#-you-do-not-want-to-write-any-code-for-your-backend]
...
*Alternatives*: [AWS AppSync](https://aws.amazon.com/appsync/), [8base](https://www.8base.com/), [Nhost](https://nhost.io/), [Supabase](https://supabase.com/), [Firebase](https://firebase.google.com/), [Amplication](https://amplication.com/)

... you want a CRUD GraphQL API without writing any code [#-you-want-a-crud-graphql-api-without-writing-any-code]
...
*Alternatives*: [Hasura](https://hasura.io/), [Postgraphile](https://www.graphile.org/postgraphile/)
