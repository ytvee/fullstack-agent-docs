# Custom model and field names (/docs/orm/prisma-client/setup-and-configuration/custom-model-and-field-names)



The Prisma Client API is generated based on the models in your [Prisma schema](/orm/prisma-schema/overview). Models are *typically* 1:1 mappings of your database tables.

In some cases, especially when using [introspection](/orm/prisma-schema/introspection), it might be useful to *decouple* the naming of database tables and columns from the names that are used in your Prisma Client API. This can be done via the [`@map` and `@@map`](/orm/prisma-schema/data-model/models#mapping-model-names-to-tables-or-collections) attributes in your Prisma schema.
