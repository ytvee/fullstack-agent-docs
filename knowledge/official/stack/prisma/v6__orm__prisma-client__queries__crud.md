# CRUD (/docs/v6/orm/prisma-client/queries/crud)
...
Create a single record using generated types [#create-a-single-record-using-generated-types]
...
<div class="videoWrapper">
  <iframe width="560" height="315" src="https://www.youtube.com/embed/QxyqR4yh1GI" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen />
</div>

Create records and connect or create related records [#create-records-and-connect-or-create-related-records]
...
The following [`findMany()`](/v6/orm/reference/prisma-client-reference#findmany) query returns *all* `User` records:
...
The following [`findFirst()`](/v6/orm/reference/prisma-client-reference#findfirst) query returns the *most recently created user* with at least one post that has more than 100 likes:
...
Get a filtered list of records [#get-a-filtered-list-of-records]
...
Filter by a single field value [#filter-by-a-single-field-value]
...
Filter by multiple field values [#filter-by-multiple-field-values]

The following query uses a combination of [operators](/v6/orm/reference/prisma-client-reference#filter-conditions-and-operators) to return users whose name start with `E` *or* administrators with at least 1 profile view:
...
Filter by related record field values [#filter-by-related-record-field-values]
...
Select a subset of fields [#select-a-subset-of-fields]
...
Select a subset of related record fields [#select-a-subset-of-related-record-fields]
...
Select distinct field values [#select-distinct-field-values]
...
Include a filtered list of relations [#include-a-filtered-list-of-relations]
...
Update a number field [#update-a-number-field]
...
Connect and disconnect related records [#connect-and-disconnect-related-records]
