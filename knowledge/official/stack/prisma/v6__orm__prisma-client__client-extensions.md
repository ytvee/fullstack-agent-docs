# Extensions (/docs/v6/orm/prisma-client/client-extensions)
...
* `model`: [add custom methods or fields to your models](/v6/orm/prisma-client/client-extensions/model)
* `client`: [add client-level methods to Prisma Client](/v6/orm/prisma-client/client-extensions/client)
* `query`: [create custom Prisma Client queries](/v6/orm/prisma-client/client-extensions/query)
* `result`: [add custom fields to your query results](/v6/orm/prisma-client/client-extensions/result)

For example, you might create an extension that uses the `model` and `client` component types.

About Prisma Client extensions [#about-prisma-client-extensions]
...
You can [share your Prisma Client extensions](/v6/orm/prisma-client/client-extensions/shared-extensions) with other Prisma ORM users, and [import Prisma Client extensions developed by other users](/v6/orm/prisma-client/client-extensions/shared-extensions#install-a-shared-packaged-extension) into your Prisma ORM project.
...
* All extended clients and the standard client communicate with the same [Prisma ORM query engine](/v6/orm/more/internals/engines).
...
Add an extension to Prisma Client [#add-an-extension-to-prisma-client]

You can create an extension using two primary ways:

* Use the client-level [`$extends`](/v6/orm/reference/prisma-client-reference#client-methods) method
...
The above examples use the [`model` extension component](/v6/orm/prisma-client/client-extensions/model) to extend the `User` model.

In your `$extends` method, use the appropriate extension component or components ([`model`](/v6/orm/prisma-client/client-extensions/model), [`client`](/v6/orm/prisma-client/client-extensions/client), [`result`](/v6/orm/prisma-client/client-extensions/result) or [`query`](/v6/orm/prisma-client/client-extensions/query)).

Name an extension for error logs [#name-an-extension-for-error-logs]
...
Type of an extended client [#type-of-an-extended-client]
...
Extending model types with Prisma.Result [#extending-model-types-with-prismaresult]
...
[Client-level methods](/v6/orm/reference/prisma-client-reference#client-methods) do not necessarily exist on extended clients. For these clients you will need to first check for existence before using.
