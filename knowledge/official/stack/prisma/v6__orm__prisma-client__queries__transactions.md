# Transactions and batch queries (/docs/v6/orm/prisma-client/queries/transactions)
...
About transactions in Prisma Client [#about-transactions-in-prisma-client]
...
A [nested write](/v6/orm/prisma-client/queries/relation-queries#nested-writes) lets you perform a single Prisma Client API call with multiple *operations* that touch multiple [*related*](/v6/orm/prisma-schema/data-model/relations) records. For example, creating a *user* together with a *post* or updating an *order* together with an *invoice*. Prisma Client ensures that all operations succeed or fail as a whole.
...
Batch/bulk operations [#batchbulk-operations]
...
Sequential Prisma Client operations [#sequential-prisma-client-operations]
...
When to use nested writes [#when-to-use-nested-writes]
...
Scenario: Sign-up flow [#scenario-sign-up-flow]
...
Nested writes FAQs [#nested-writes-faqs]

Why can't I use the $transaction([]) API to solve the same problem? [#why-cant-i-use-the-transaction-api-to-solve-the-same-problem]
...
Nested writes support nested updates, but updates are not dependent writes - should I use the $transaction([]) API? [#nested-writes-support-nested-updates-but-updates-are-not-dependent-writes---should-i-use-the-transaction-api]
...
Can I perform multiple nested writes - for example, create two new teams and assign users? [#can-i-perform-multiple-nested-writes---for-example-create-two-new-teams-and-assign-users]
