# MCP server (/docs/v6/postgres/integrations/mcp-server)
...
* `Prisma-Postgres-account-status`: Checks your authentication status with [Prisma Console](https://console.prisma.io/?utm_source=docs-v6\&utm_medium=content\&utm_content=postgres) via the `platform auth show --early-access` command.
...
* `Prisma-Login`: Authenticates with [Prisma Console](https://console.prisma.io/?utm_source=docs-v6\&utm_medium=content\&utm_content=postgres) via the `platform auth login --early-access` command.
